...
def for_leaderboard(data):
    _ = data.groupby('user')
    df = pd.concat([_.head(1), _.tail(1)]).drop_duplicates().sort_values('user').reset_index(drop=True).dropna()
    ...
    leaderboard = df.groupby('user').tail(1).dropna(axis=0)[['user', 'lb_pct', 'days_since']].sort_values('lb_pct').reset_index(drop=True)
